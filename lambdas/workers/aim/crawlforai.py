...
def upload_to_new_bucket_structure(content, job_id, product_id, query_id, content_type='application/json', file_extension='json', session_id=None):
    ...
    try:
        ...
        s3_key = f"{brand_name_safe}/{job_id}/{session_id_safe}/{product_name_safe}/{query_text_safe}/aim/response.{file_extension}"
        ...
        if result and result.get('success') and not result.get('failure_type') == 'failure':
            try:
                ...
                new_bucket_path = upload_to_new_bucket_structure(
                    aim_result, job_id, product_id, query_id, 'application/json', 'json', session_id
                )
                if new_bucket_path:
                ...
                if external_sources > 0:
                    ...
                    if citations:
                        ...
                        citation_invocation_success = invoke_citation_scraper_lambda(
                            unique_citations, job_id, query_id, product_id, query, task_id, session_id
                        )
