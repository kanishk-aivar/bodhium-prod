...
class DatabaseManager:
    ...
    def _create_tables(self):
        ...
        -- Create products table (matching db_schema.md)
        CREATE TABLE IF NOT EXISTS products (
            product_id BIGSERIAL PRIMARY KEY,
            product_hash VARCHAR(256),
            product_data JSONB,
            source_url TEXT,
            ...
            with self.get_connection() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    for product in products:
                        ...
                        cur.execute("SELECT product_id FROM products WHERE product_hash = %s", (product_hash,))
                        ...
                        if existing_product:
                            ...
                            # Insert new product with new schema
                            cur.execute("""
                                INSERT INTO products (
                                    product_hash, product_data, source_url, brand_name
                                ) VALUES (%s, %s, %s, %s)
                                RETURNING product_id
                            """, (
                                product_hash,
                                json.dumps(product),  # Store all product data as JSONB
                                product.get('source_url', ''),
                                product.get('brand', '') or os.getenv('BRAND_NAME', '')
                            ))
                            result = cur.fetchone()
...
class EnhancedWebCrawler:
    def __init__(self, model_name: str, output_dir: str, job_id: str = None, brand_name: str = None):
        ...
        if brand_name:
            logger.info(f"🏷️ Brand Name: {brand_name}")
    ...
    def setup_job_in_database(self, url: str, job_id: str = None) -> str:
        ...
        # Use brand from constructor first, then environment variable, fallback to domain extraction
        brand = self.brand_name or os.getenv("BRAND_NAME") or domain.split('.')[0].title()
        logger.info(f"📋 Using brand: {brand} (from {'constructor' if self.brand_name else 'environment' if os.getenv('BRAND_NAME') else 'domain extraction'})")
...
"""
    ...
    def deduplicate_products(self, products: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for product in products:
            # Prefer 'productname', fallback to common alternatives if present
            raw_name = product.get('productname') or product.get('name') or product.get('title')
            name = to_clean_string(raw_name).lower()
            price = to_clean_string(product.get('current_price'))
...
async def main():
    ...
    parser.add_argument('--brand-name', type=str, help='Brand name for the crawl job')
    ...
    if not event_data:
        ...
        if meaningful_args:
            try:
                ...
                logger.info(f"📋 Parsed arguments - URL: {url}, Job ID: {job_id}, Brand: {brand_name}, Output dir: {output_dir}")
            except SystemExit:
            ...
            # No meaningful arguments provided, check environment variables
            logger.info("📋 No meaningful arguments provided, checking environment variables")
            ...
            if url:
                logger.info(f"✅ Found URL in environment variable CRAWL_URL: {url}")
            if job_id:
                logger.info(f"✅ Found Job ID in environment variable AWS_BATCH_JOB_ID: {job_id}")
            if brand_name:
                logger.info(f"✅ Found Brand Name in environment variable BRAND_NAME: {brand_name}")
            if output_dir:
                logger.info(f"✅ Found Output Directory in environment variable OUTPUT_DIR: {output_dir}")
            
            # If brand_name is not provided via arguments, try to get it from environment
            if not brand_name:
                brand_name = os.getenv('BRAND_NAME')
                if brand_name:
                    logger.info(f"✅ Found Brand Name in environment variable BRAND_NAME: {brand_name}")
        ...
        if 'body' in event_data:
            try:
                ...
                if brand_name:
                    logger.info(f"✅ Found Brand Name in request body: {brand_name}")
        ...
        if not url and 'queryStringParameters' in event_data and event_data['queryStringParameters']:
            ...
            brand_name = event_data['queryStringParameters'].get('brand_name')
            if url:
                logger.info(f"✅ Found URL in query parameters: {url}")
            if job_id:
                logger.info(f"✅ Found Job ID in query parameters: {job_id}")
            if brand_name:
                logger.info(f"✅ Found Brand Name in query parameters: {brand_name}")
        ...
        if not brand_name:
            brand_name = event_data.get('brand_name') or os.getenv("BRAND_NAME")
            if brand_name:
    ...
    if not job_id:
        error_msg = "Job ID is required but not provided. Check AWS_BATCH_JOB_ID environment variable or event data."
        logger.error(f"❌ {error_msg}")
        raise ValueError(error_msg)
    
    # Debug logging
    logger.info(f"📋 Final parameters - URL: {url}, Job ID: {job_id}, Brand: {brand_name}, Output dir: {output_dir}")
